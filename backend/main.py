...
app = FastAPI(title="Executive Engine V16")
...
@app.get("/")
def root():
    return {"status": "ok", "service": "Executive Engine V16"}
...
async def run(req: Req):
    ...
    memory_text = "\n".join([
        f"Previous: {m.get('input','')} | Decision: {m.get('decision','')} | Next: {m.get('next_move','')}"
        for m in memory
    ]) or "No prior memory."
