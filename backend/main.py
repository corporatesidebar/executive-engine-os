...
VERSION = "35150-selective-run-quality-on-v35130r"
...
{
  "next_move": "the next operational move, specific to the user's command",
  "decision": "clear executive decision, not generic",
  "action_steps": ["3-7 concrete steps with owner/action/object"],
  "ready_assets": ["specific assets produced or ready now"],
  "risk": "specific business risk",
  "priority": "High | Medium | Low",
  "recommended_command": "the next command the operator should run",
  "what_to_do_now": "one immediate action, under 22 words",
  "actions": ["same concrete steps as action_steps for backward compatibility"],
...
CONTRACT RULES:
- next_move, decision, action_steps, ready_assets, risk, priority, and recommended_command are mandatory.
- Do not return vague placeholder language.
- ready_assets must be an array. If a draft, plan, brief, proposal, email, or checklist is created, name it in ready_assets.
- recommended_command must be a practical next command the user can paste into Executive Engine OS.
- Preserve backward-compatible keys actions, asset, follow_up, workspace, and operator_state when available.
...
def _contract_text(value, fallback=""):
    if value is None:
        return fallback
    ...
    if isinstance(value, dict):
        try:
            ...
            return clean if clean and clean != "{}" else fallback
        except Exception:
            return fallback
...
def _split_contract_steps(value):
    if isinstance(value, list):
        return [_contract_text(item, "").strip() for item in value]
    if isinstance(value, str):
        return [line.strip(" -•\t") for line in re.split(r"\n|\d+\.|;", value) if line.strip(" -•\t")]
    return []


def _contract_action_steps(payload, req=None, input_text=""):
    command = input_text or getattr(req, "input", "") or "the executive command"
    candidates = []
    for key in ("action_steps", "actions", "tasks"):
        candidates.extend(_split_contract_steps(payload.get(key)))
    fallback = [
        f"Define the winning outcome for: {command[:120]}.",
        "Name the owner, deadline, buyer or stakeholder, and required decision point.",
        "Create the first executive asset needed to move the work forward.",
        "Send the follow-up or decision request with one clear call to action.",
        "Track the blocker, risk, and next checkpoint before expanding scope.",
    ]
    clean, seen = [], set()
    for step in candidates + fallback:
        step = _contract_text(step, "").strip()
        if not step:
            continue
        sig = re.sub(r"\s+", " ", step.lower())
        if sig in seen:
            continue
        seen.add(sig)
        clean.append(step)
...
def _contract_ready_assets(payload):
    assets = []
    raw = payload.get("ready_assets") or payload.get("assets")
    if isinstance(raw, list):
        assets.extend(_contract_text(item, "").strip() for item in raw)
    elif isinstance(raw, str) and raw.strip().lower() not in {"", "none", "n/a", "na", "not available"}:
        assets.append(raw.strip())
    asset = payload.get("asset")
    if isinstance(asset, dict):
        title = _contract_text(asset.get("title"), "").strip()
        atype = _contract_text(asset.get("type"), "asset").strip()
        content = _contract_text(asset.get("content"), "").strip()
        if title:
            assets.append(f"{title} ({atype})" if atype else title)
        elif content:
            assets.append("Executive draft asset")
    clean, seen = [], set()
    for item in assets:
        item = _contract_text(item, "").strip()
        if not item:
            continue
        sig = item.lower()
        if sig not in seen:
...
def _contract_priority(value, fallback="High"):
    text = _contract_text(value, fallback).lower()
    if "low" in text:
        return "Low"
...
def enforce_v35150_run_contract(payload, req=None, router=None, provider_used=None, input_text=""):
    if not isinstance(payload, dict):
        payload = {}
    command = input_text or getattr(req, "input", "") or "the executive command"
    mode = getattr(req, "mode", "execution") if req is not None else "execution"
    router = router if isinstance(router, dict) else payload.get("router") if isinstance(payload.get("router"), dict) else {}

    action_steps = _contract_action_steps(payload, req=req, input_text=command)
    ready_assets = _contract_ready_assets(payload)

    payload["next_move"] = _contract_text(
        payload.get("next_move") or payload.get("what_to_do_now"),
        f"Lock the outcome, owner, deadline, and first asset for: {command[:140]}."
    )
    payload["decision"] = _contract_text(
        payload.get("decision"),
        "Move forward with one controlled executive workflow instead of leaving this as open-ended discussion."
    )
    payload["action_steps"] = action_steps
    payload["actions"] = action_steps
    payload["ready_assets"] = ready_assets
    payload["risk"] = _contract_text(
        payload.get("risk"),
        "The work loses momentum if the owner, deadline, decision path, and required asset are not made explicit."
    )
    payload["priority"] = _contract_priority(payload.get("priority"), router.get("urgency", "High"))
    payload["recommended_command"] = _contract_text(
        payload.get("recommended_command"),
        f"Turn this into a {mode} plan with owner, deadline, decision path, risks, and ready asset."
    )
    payload["what_to_do_now"] = _contract_text(payload.get("what_to_do_now"), payload["next_move"])
    payload["follow_up"] = _contract_text(payload.get("follow_up"), "Confirm the owner, deadline, success metric, and next checkpoint.")
    if provider_used:
        payload["provider_used"] = provider_used
    if isinstance(router, dict):
        router = dict(router)
        router["contract_version"] = "v35150-selective-run-quality"
        payload["router"] = router
    return payload

def normalize(data, req, router, provider_used):
    data = data if isinstance(data, dict) else {}
    actions = data.get("action_steps") or data.get("actions", [])
    ...
    payload = {
        ...
        "action_steps": [str(a) for a in actions if str(a).strip()][:8] or ["Confirm objective.", "Create asset.", "Send follow-up."],
        "ready_assets": data.get("ready_assets") if isinstance(data.get("ready_assets"), list) else [],
        "risk": str(data.get("risk") or "Risk not specified."),
        "priority": priority,
        "recommended_command": str(data.get("recommended_command") or "Continue this workflow with owner, deadline, decision path, and ready asset."),
    ...
    return enforce_v35150_run_contract(payload, req=req, router=router, provider_used=provider_used)
...
def v35080_clean_response_payload(payload, input_text=""):
    ...
    payload["provider_used"] = payload.get("provider_used") or "openai-first:stabilization-v35090"
    return enforce_v35150_run_contract(payload, router=payload.get("router"), provider_used=payload.get("provider_used"), input_text=input_text)
...
def test_report_json():
    report = {
        ...
        "timestamp": now(),
        "backend": "live",
        "output_quality_features": ["client-ready proposal fallback", "workspace reset endpoints", "clean identity extraction", "stronger assets/tasks/follow-up generation", "V35150 required /run contract enforcement", "recommended_command normalization", "ready_assets normalization"],
        "routes_restored": [
        ...
        "required_run_contract": REQUIRED_RUN_CONTRACT,
        "schema": {
            "next_move": "string",
            "decision": "string",
            "action_steps": "array",
