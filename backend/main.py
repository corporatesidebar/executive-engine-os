...
APP_NAME = "Executive Engine OS V90"
MODEL = os.getenv("OPENAI_MODEL", "gpt-4o-mini")
TIMEOUT = float(os.getenv("OPENAI_TIMEOUT_SECONDS", "35"))
...
@app.get("/debug")
async def debug(): return {"ok": True, "version": "V90", "routes": ["/", "/health", "/run", "/robots.txt"], "model": MODEL}

@app.post("/run")
async def run(req: RunRequest):
    system = """You are Executive Engine OS V90. Act like an elite CEO/COO/President operator.
...
- Do not give shallow short answers. Every field must add new useful context.
- Each action must be executable, specific, and contain enough detail to act immediately.
- Write fuller responses: what to do, why it matters, what risk to watch, what to ignore, and what the user should do next.
- Avoid generic statements like "clarify goals" unless paired with an exact method.
- The response should feel like an expensive executive operator/advisor, not a basic chatbot.
- No generic advice.
